	...
	<div class="col-lg-12 col-sm-12 col-xs-12">
		<div class="well with-header with-footer">
			...
			<div class="widget-main">
				<form class="form-horizontal" role="form" action="/tp3/Admin/Brand/add" method="post">
					<!-- 分类名称 -->
					<div class="form-group">
						<label for="brand_name" class="col-sm-2 control-label no-padding-right">品牌名称</label>
						<div class="col-sm-10">
							<input type="text" value="<?php echo ((isset($data["brand_name"]) && ($data["brand_name"] !== ""))?($data["brand_name"]):''); ?>" class="form-control" name="brand_name" id="brand_name" placeholder="品牌名称">
						</div>
					</div>	
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<a href="javascript:void(0);" class="btn btn-darkorange" onclick="btn.uploader(this,'logo',true)">
								<i class="fa fa-picture-o"></i>
								上传图片
							</a>
							<?php if(!empty($data["logo"])): ?><ul class="pic-list-body">
								<li>
									<img src="/tp3/<?php echo ($data["logo"]); ?>" width="100" height="100">	
					...
					<!-- 简介 -->
					<div class="form-group">
						<label for="abstract" class="col-sm-2 control-label no-padding-right">品牌简介</label>
						<div class="col-sm-10">
							<textarea class="form-control" rows="6" name="abstract" id="abstract" placeholder="品牌简介"><?php echo ((isset($data["abstract"]) && ($data["abstract"] !== ""))?($data["abstract"]):''); ?></textarea>
						</div>
					</div>
	...
	var btn = {
			...
			$('input[name="myfile"]').change(function(){
				...
				$.ajax({
					...
					success: function(data) {
						...
						var li = '<li>'+
								 '	<img src="/tp3/'+data.path+'" width="100" height="100">'+
		...
		spec_add:function(p){
			...
			if($('.modal-footer .loading').length == 0){
				...
				$('.modal-footer').prepend('<span class="loading"><img id="loading" src="/tp3/Public/Admin/images/loading1.gif" width="30" style="margin-left:10px;"></span>');
			}
