	...
	<div class="col-lg-12 col-sm-12 col-xs-12">
		<div class="well with-header with-footer">
			<div class="header bordered-sky">
				...
				<a class="btn btn-darkorange  btn-xs pull-right" href="<?php echo U('add');?>">
					<i class="fa fa-plus"></i>
					<?php echo str_replace('管理','',$page_title);?>添加
				</a>
			</div>
			<table class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						<th>
							<i class="fa fa-list-ol"></i> id
						</th>
						<th class="hidden-xs">
							<i class="fa  fa-font"></i> 规格名称
						</th>
						<th>
							<i class="fa fa-list"></i> 选项
						</th>
						<th>
				...
				<tbody>
					<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
						<td><?php echo ($v["id"]); ?></td>
						<td class="hidden-xs"><?php echo ($v["spec_name"]); ?></td>
						<td>
							<?php if(is_array($v["SpecItems"])): foreach($v["SpecItems"] as $key=>$s): if(!empty($key)): ?>、<?php endif; ?>
							<?php echo ($s["spec_value"]); endforeach; endif; ?>
						</td>
						<td>
							<a href="<?php echo U('edit',['id'=>$v['id']]);?>" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i> 编辑</a>
							<a href="javascript:;" onclick="btn.del('<?php echo U('del',['id'=>$v['id']]);?>')" class="btn btn-maroon btn-xs"><i class="fa fa-trash-o"></i> 删除</a>
						</td>
			...
			<div class="row DTTTFooter" style="margin-top:10px;">
				<div class="col-sm-6 col-lg-6 col-xs-6">
					<div class="dataTables_info" id="simpledatatable_info" role="alert" aria-live="polite" aria-relevant="all">
						当前第 <?php echo ((isset($_GET['p']) && ($_GET['p'] !== ""))?($_GET['p']):"1"); ?> 页, 每页显示 <?php echo ($listRows); ?> 条, 共 <?php echo ($count); ?> 条数据
					</div>
				</div>
				<div class="col-sm-6 col-lg-6 col-xs-6">
