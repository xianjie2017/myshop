	...
	<div class="col-lg-12 col-sm-12 col-xs-12">
		<div class="well with-header with-footer">
			...
			<div class="widget-main">
				<form class="form-horizontal" role="form" action="/tp3/Admin/ArticleCategory/add" method="post">
					<!-- 分类名称 -->
					<div class="form-group">
						<label for="goods_name" class="col-sm-2 control-label no-padding-right">分类名称</label>
						<div class="col-sm-10">
							<input type="text" value="<?php echo ((isset($data["cate_name"]) && ($data["cate_name"] !== ""))?($data["cate_name"]):''); ?>" class="form-control" name="cate_name" id="cate_name" placeholder="分类名称">
						</div>
					</div>	
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<select id="pid" name="pid" style="width:100%;">
								<option value="0" />请选择上级分类</option>
								<?php if(is_array($parent)): foreach($parent as $key=>$p): ?><option value="<?php echo ($p["id"]); ?>" <?php if(($p['id']) == $data['pid']): ?>selected<?php endif; ?> />├<?php echo str_repeat('──',$p['level'] - 1); echo ($p["cate_name"]); ?></option><?php endforeach; endif; ?>
							</select>
						</div>
					</div>
